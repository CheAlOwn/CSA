...
public class AuthorizationController {
    ...
    private boolean checkUser(String login, String password) throws SQLException {
        ResultSet rs = DBConnector.query("SELECT u.id, login, password_hash, e.second_name, e.first_name , e.patronymic, post_name, id_employee \n" +
                "FROM \"user\" u\n" +
        ...
        if (rs.next()) {
            if (SecurityUtil.verifyPassword(password, rs.getString("password_hash"))) {
                StaticObjects.setCurrentUser(new User(
                        rs.getInt("id"),
                        rs.getString("login"),
